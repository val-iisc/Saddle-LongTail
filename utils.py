...
def save_numpy(args, clss_idx, density_eigen, density_weight):
    eigen_filename = '%s/%s/%s_density_eigen' % (args.root_model, args.store_name, clss_idx)
    weights_filename = '%s/%s/%s_density_weights' % (args.root_model, args.store_name, clss_idx)
    print(f"Saving density eigen as numpy for {clss_idx}")
    np.save(eigen_filename, density_eigen)
    print(f"Saving density weights as numpy for {clss_idx}")
...
def save_checkpoint_new(args, state, is_best, save_freq):
